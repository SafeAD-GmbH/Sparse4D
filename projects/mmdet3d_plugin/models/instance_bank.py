...
class InstanceBank(nn.Module):
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        if torch.onnx.is_in_onnx_export():
            anchor = self.anchor.clone()[None, ...].repeat(batch_size, 1, 1)
            instance_feature = self.instance_feature.clone()[None, ...].repeat(batch_size, 1, 1)
        else:
        ...
        if (self.cached_anchor is not None and batch_size == self.cached_anchor.shape[0]):
            ...
            if (
                self.anchor_handler is not None and dn_metas is not None and
