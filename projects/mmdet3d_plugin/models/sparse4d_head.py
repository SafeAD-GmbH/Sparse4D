...
class Sparse4DHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        if operation_order is None:
            operation_order = ["temp_gnn", "gnn", "norm", "deformable", "norm", "ffn", "norm", "refine"] * num_decoder
    ...
    def forward(
        ...
        if dn_metas is not None:
            ...
            if dn_anchor.shape[-1] != anchor.shape[-1]:
                ...
                dn_anchor = torch.cat(
                    [
                        dn_anchor,
                        dn_anchor.new_zeros(batch_size, num_dn_anchor, remain_state_dims),
                    ],
                    dim=-1,
                )
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                anchor, cls, qt = self.layers[i](
                    ...
                    return_cls=(
                        self.training or len(prediction) == self.num_single_frame_decoder - 1 or
        ...
        if dn_metas is not None:
            ...
            quality = [x[:, :num_free_instance] if x is not None else None for x in quality]
            output.update(
                {
    ...
    def loss(self, model_outs, data, feature_maps=None):
        ...
        for decoder_idx, (cls, reg) in enumerate(zip(dn_cls_scores, dn_reg_preds)):
            ...
            reg_loss = self.loss_reg(
                reg.flatten(end_dim=1)[dn_valid_mask][dn_pos_mask][..., :len(self.reg_weights)],
                dn_reg_target,
                avg_factor=num_dn_pos,
                weight=reg_weights,
